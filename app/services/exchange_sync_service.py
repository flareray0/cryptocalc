...
class ExchangeSyncService:
    ...
    def save_connection(self, *, api_key: str | None, api_secret: str | None, base_url: str | None = None) -> dict[str, Any]:
    ...
    def sync(self, *, symbols: list[str], start_time_ms: int | None = None, end_time_ms: int | None = None) -> dict[str, Any]:
        client = self._client()
        settings = load_settings()
        ...
        incoming = cast(list[NormalizedTransaction], sync_result["transactions"])
        warnings = cast(list[str], sync_result.get("warnings", []))

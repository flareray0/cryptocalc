...
def _reconciliation_root() -> Path:
    root = get_paths().app_data / "balance_reconciliation_runs"
    root.mkdir(parents=True, exist_ok=True)
    return root


def _run_path(run_id: str) -> Path:
    return _reconciliation_root() / f"{run_id}.json"


def _index_path() -> Path:
    return _reconciliation_root() / "index.json"
...
def save_balance_reconciliation(payload: dict[str, Any]) -> None:
    ...
    index.append(
        {
            ...
            "method_reference": payload.get("method_reference"),
...
def load_balance_reconciliation(run_id: str) -> dict[str, Any] | None:
    path = _run_path(run_id)
    if not path.exists():
        return None
...
def load_latest_balance_reconciliation(
    *,
    ...
    for row in rows:
        ...
        if end_year is not None and row.get("end_year") != end_year:
            continue
        return load_balance_reconciliation(row["run_id"])
    return None
